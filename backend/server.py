...
<body>
  ...
  <script>
    ...
    async function connect() {
      ...
      ws = new WebSocket(`${proto}://${location.host}/ws/browser${location.search}`);
...
def create_app() -> FastAPI:
    ...
    async def browser_websocket(websocket: WebSocket):
        ...
        logger.info("Browser websocket connected, elder_id={}", elder_id)
        transport = create_browser_transport(websocket)
        await run_bot_session(transport, "browser", False, elder_id=elder_id)
